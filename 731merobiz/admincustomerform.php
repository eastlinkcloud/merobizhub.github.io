...
echo '<script>
   setTimeout(function() {
     ...
     if (move_uploaded_file($profile_img_tmp, $profile_img_path) && move_uploaded_file($docs_img_tmp, $docs_img_path)) {
        ...
        $insert_query = "INSERT INTO customer (profile_img, docs_img, customer_name, category, product, price, wardno, streetno, province, district, municipality, mobilenumber, email, status, latitude, longitude, salesmanager_id) 
        VALUES (:profile_img, :docs_img, :customer_name, :category, :product, :price, :wardno, :streetno, :province, :district, :municipality, :mobilenumber, :email, :status, :latitude, :longitude, :salesmanager_id)";
...
<div class="container-fluid">
    <!-- Form part -->
    <p class="h3 mb-4 text-gray-800">Customer Registration</p>
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="form-horizontal mx-auto col-10 col-md-10 col-lg-10 shadow-lg my-4 p-4 m-4 needs-validation" enctype="multipart/form-data" novalidate>
        <!-- Rest of your form --> <div class="form-row mb-4">
            ...
            <div class="col-xl-4 position-relative">
                Select SalesPoint*
                <select name="salesmanager_id" class="form-select" id="validationTooltip04" required>
                    <option selected disabled value="">Select SalesPoint*</option>
                    <?php
                    $query = "SELECT * FROM salesmanager WHERE status='Active'";
                    $stmt = $conn->prepare($query);
                    $stmt->execute();
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    foreach ($rows as $row) {
                        echo "<option value='{$row['salesmanager_id']}'>{$row['salesmanager_name']}</option>";
                    }
                    ?>
                </select>
                    <div class="invalid-feedback feedback-pos">
                        Please select SalesPoint.
                    </div>
            </div>
            <!-- customer name -->
            <div class="col-xl-4">
                Customer Name*
                <input type="text" name="customer_name" id="validationTooltip01" placeholder="Customer Name*" class="form-control" required>
    <div class="invalid-feedback feedback-pos">
      Input required field.
    </div>
            </div>
            <div class="col-xl-4">
            Email* <input type="email" name="email" placeholder="Your email ID.." class=" form-control" id="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required>
